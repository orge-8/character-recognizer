...
def build_compress_prompt(cards: "Sequence[str]") -> str:
    ...
    * **一条卡只写一类特征**——原卡片是按类分的（一条讲发色发型、一条讲眼睛……），整理必须
      保持这个结构。真机出过事：只写了"合并同一条特征的不同说法"，模型就把**造型**当成分组键，
      把每张图的发色+眼睛+服装压成一条大杂烩，整库只剩"眼睛"一类，连缺口提示都反过来误报。
    * **不同造型不许合并**——"蓝色短发"与"浅蓝色长直发"是两种造型。合并就丢了造型多样性，
      而识别时正是靠它判断"今天这张图属于哪个形态"。
    * **细节不许丢**——发饰的形状与颜色、服装配件、配色往往就是区分两个角色的唯一线索，
      "精简"最先抹掉的偏偏就是它们。

    换句话说：冗余只是浪费配额，丢细节或丢结构直接降低准确率。所以宁可压得少。
    """
    ...
    return (
        ...
        '{"appearance_cards":["..."]}。\n'
        "规则：\n"
        "1. 只合并**同一类特征**的不同说法：好几条都在讲发色发型 → 合成一条最完整的发色发型。\n"
        "2. **一条卡片只写一类特征**（发色发型 / 眼睛 / 配饰 / 服装），绝不要把多类写进同一条。\n"
        "   合格：「红棕眼眸，佩戴红白色头戴式耳机」\n"
        "   不合格：「深棕短发带呆毛；红棕眼眸；红白露肩上衣配黑长筒袜」——把三类硬拼成一条\n"
        "3. **不同造型各自成条，绝不合并**：短发与长直发是两种造型，不同服装也是不同造型。\n"
        "4. 合并时**保留全部区分性细节**（发饰的形状与颜色、服装配件与配色）。"
        "不要因为「要精简」就丢细节——这些细节正是识别角色时用的。\n"
        "5. 不要新增卡片里没有的信息，不要推测角色名或作品。\n"
        "6. 每条不超过 55 字；总数不超过 15 条，也不少于 3 条。\n"
        f"外观卡：\n{body}"
    )
