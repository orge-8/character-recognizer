...
def download_image(url: str, *, timeout_seconds: float = 15.0, max_bytes: int = 8_388_608) -> bytes | None:
    """直接从 URL 下载图片（部分适配器只保留 url 而不带 base64）。

    不需要任何 Host 能力，普通 HTTP 即可。失败一律返回 None。
    """
    ...
    try:
        request = Request(str(url), headers={"User-Agent": "MaiBot-character-recognizer/1.0"})
